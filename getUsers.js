...
// токен для подключения к vc
const token = '';


// прокручиваем цикл с задержкой
let i = 0;      // начало отсчета (с какого номера пользователя начать загружать)
let count = 600000;  // повторений (предполагаемое количество пользователей в базе)
...
function looperCallback(i, ready){
    ...
    // запрос пользователя
    let reqOptions = {
        url: 'https://api.vc.ru/v1.8/user/'+i,
        headers: {
        'User-Agent': 'request',
    ...
    request(reqOptions, (error, response, body) => {
        ...
        if (!error && response.statusCode == 200) {
          ...
          // отчет 
          log.info('Ответ получен произошла ошбка: ', JSON.stringify(error))
